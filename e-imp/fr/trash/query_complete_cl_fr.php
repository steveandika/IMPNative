...
<form id="printXLS" method="post" Action="query_complete_cl_fr_xls">
  <input type="hidden" name="instart" value="<?php echo $tgl1?>" />
  <input type="hidden" name="inlast" value="<?php echo $tgl2?>" />
  <input type="hidden" name="loc" value="<?php echo $loc?>" />
  <input type="hidden" name="do" value="<?php echo $_POST['do']?>" />
  <button type="submit">Export To XLS</button>
</form>
<div class="height-10"></div>
<h3>Summary C/R and C/Cleaning</h3> 			
 ...
 <table class="w3-table w3-bordered">
  <thead>
    <tr> 
      ...
      <th colspan="3" style="text-align: center">Estimate</th>      
...
<!--      <th rowspan="2">Approved On</th>-->      
    ...
    <tr> 
      ...
      <th style="text-align:right">M/H</th>
      <th style="text-align:right">Labor</th>
      <th style="text-align:right">Mtrl</th>
...
<script language="php">  
  ...
  if($_POST["do"]=="REPAIR") {
    $query="Select a.estimateID, a.containerID, b.Size, b.Type, b.Height, b.Constr, Format(c.gateIn,'yyyy-MM-dd') As TanggalIn, 
				   ...
				   IsNull(f.nilaiDPP,0) As cleaningDPP, IsNull(g.materialValue,0) As materialCleaning	  
            From   RepairHeader a With (NOLOCK)
			...
			Left Join CleaningDetail g On g.CleaningID=f.CleaningID
    ...
    while($arr=mssql_fetch_array($result)) {
	 if($arr["nilaiDPP"] != $arr["cleaningDPP"]) { 	
	  ...
	  $design=$design.'<tr>
			            ...
			            <td style="text-align: right">'.number_format($materialRepair,2,",",".").'</td>
  ...
  if($_POST["do"]=="CLEANING") {
    ...
    $query="Select g.estimateID, a.containerID, b.Size, b.Type, b.Height, b.Constr, Format(c.gateIn,'yyyy-MM-dd') As TanggalIn, 
		           ...
		           Format(g.estimateDate,'yyyy-MM-dd') As tanggalEst, Format(a.CleaningDate, 'yyyy-MM-dd') As CleaningDate,
                   0 As totalHour, 0 As totalLabor, a.nilaiDPP, '' As tanggalSPK, 
				   Format(g.tanggalApprove, 'yyyy-MM-dd') As tglApprove, IsNull(f.materialValue,0) As materialCleaning	  
            From   CleaningHeader a with (NOLOCK)
            Inner Join containerLog b On b.ContainerNo=a.ContainerID
		    Inner Join containerJournal c On c.NoContainer=a.ContainerID And c.bookInID=a.BookID And CCleaning Is Not Null
		    Inner Join tabBookingHeader d On d.bookID=a.BookID
		    Inner Join m_Customer e On e.custRegID=d.principle
			Inner Join CleaningDetail f On f.CleaningID=a.CleaningID
		    Left Join repairHeader g On g.bookID=a.bookID And g.ContainerID=a.ContainerID And g.tanggalApprove != '1900-01-01'
		    Where  (CCleaning Between '$tgl1' And '$tgl2') And c.workshopID='$loc' And CCleaning Is Not Null 
		    Order By e.CompleteName, c.gateIn,  b.Size, b.Type";
  //echo $query;
    $result=mssql_query($query);  
    while($arr=mssql_fetch_array($result)) {
	  ...
	  if($validToView==1) {
		...
		$Labor_Cleaning=$arr["nilaiDPP"] -$arr["materialCleaning"];
        ...
        $totalMtrl_Cleaning=$totalMtrl_Cleaning+$arr["materialCleaning"];
	    ...
	    $design=$design.'<tr>
			              ...
			              <td style="text-align: right">'.number_format($arr["materialCleaning"],2,",",".").'</td>
