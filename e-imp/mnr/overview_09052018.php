      ...
      if($_POST["formID"] == 'fcontEvent' && $_POST['discharge'] == '') {
        ...
        $do = "Update containerJournal Set gateIn = '$dtmin', GIPort='$dateInPort', workshopID='$workshop' Where bookInID = '$kodeBooking' And NoContainer = '$kywrd';			 		
	           ...
	           Insert Into userLogAct(userID, dateLog, DescriptionLog) 
	                           Values('".$_SESSION['uid']."', GETDATE(),CONCAT('UPDATE EVENT LOG ',' $kywrd','_','$kodeBooking')); ";
...
/*	   
		...
		{	   
	      $do = "Update containerJournal Set tanggalSurvey = '$dtmsurvey', isCleaning = 1, isRepair = 1, pendingRemark = '$remark', Surveyor = '$surveyor'
		          Where bookInID = '$kodeBooking' And NoContainer = '$kywrd';
		   	     ...
		   	     Insert Into userLogAct(userID, dateLog, DescriptionLog) 
	                           Values('".$_SESSION['uid']."', GETDATE(),CONCAT('UPDATE SURVEY LOG ','$kywrd','_','$kodeBooking')); ";
	      $result = mssql_query($do);		  
	  ...
	  if($_POST["formID"] == "fcontCleaning" && $_POST['discharge'] == '') {
        ...
        $query = "If Not Exists(Select * From CleaningHeader Where containerID='$kywrd' And bookID='$kodeBooking') Begin
                     ...
                     Insert Into CleaningDetail(cleaningID, locationID, materialValue, Remarks, repairID) 
                                         Values(@NewDraft, '$location', $dpp, '$remark', '$cleaning');
	    ...
	    $do="Update containerJournal Set cleaningType='$cleaning' Where NoContainer='$kywrd' And bookInID='$kodeBooking';
	         Insert Into userLogAct(userID, dateLog, DescriptionLog) 
	                         Values('".$_SESSION['uid']."', GETDATE(), CONCAT('UPDATE CLEANING LOG ','$kywrd','_','$kodeBooking')); ";
	    $result = mssql_query($do);
	  ...
	  if($_POST["formID"] == 'fcontRepair' && $_POST["discharge"] == '') {
	    ...
	    $do = '';	
        if(trim($NoEst) != '') {	   
	      $do = "If Not Exists(Select estimateID From RepairHeader Where estimateID='$NoEst') Begin 		  
	    ...
	    if(trim($cr) != '') {
	      $do = "Update containerJournal Set CRDate = '$cr' Where bookInID = '$kodeBooking' And NoContainer = '$kywrd';
	             Insert Into userLogAct(userID, dateLog, DescriptionLog) 
	                             Values('".$_SESSION['uid']."', GETDATE(),CONCAT('UPDATE COMPLETE REPAIR ','$kywrd','_','$kodeBooking')); ";
	      $res = mssql_query($do);	  
    ...
    if($_POST["formID"] == 'fcontRepair' || $_POST["formID"] == "fcontCleaning") {
	  ...
	  if(strtotime($tglCR) < strtotime($tglCleaning)) { $avcond=$cldate; }
...
<div class="w3-container">
  ...
  <div class="w3-container" style="background:#f8f9f9">
	...
	<div class="w3-row-padding">
	  ...
	  <div class="w3-half"> <!-- basic info and captured event -->
	    <label style="font: 600 15px/25px Rajdhani, Helvetica, sans-serif;">Container Profile and Captured Event</label>
		<div class="height-10" style="border-top:1px solid #27ae60"></div>
        <div class="w3-row-padding" >
	    ...
	    <div class="w3-row-padding">
         <div class="w3-half ">Hamparan/Workshop In</div>
	    ...
	    <div class="w3-row-padding" >
         <div class="w3-half ">Hamparan/Workshop Out</div>
		...
		<div class="w3-row-padding">
		 <div class="w3-half">
          <form id="fcntEvent" method="post">
			...
			<button type="submit" class="w3-button w3-light-grey main-button_light-blue">Event Log</button>
		  </form>
		 </div>
		 <div class="w3-half">		 
          <?php		 		    
			...
			else { echo '<button type="submit" disabled class="w3-button w3-light-grey main-button_light-blue">Remove</button>';  }	
		  ?>	
		 </div>
		</div>		
	  </div>

	  <div class="w3-half"> <!-- survey log -->
	    <label style="font: 600 15px/25px Rajdhani, Helvetica, sans-serif;">Survey Log</label>
		<div class="height-10" style="border-top:1px solid #27ae60"></div>
	    <div class="w3-row-padding">
		...
		<div class="w3-container">
 	      <form id="fcntsurvey" method="post">
			<input type="hidden" name="noCnt" value="<?php echo $kywrd?>" />
			<input type="hidden" name="bookID" value="<?php echo $kodeBooking?>" />
			<button type="submit" class="w3-button w3-light-grey main-button_light-blue">Survey Log</button>
		  </form>
		</div>	  
	  </div>	  
	</div>
	<div class="height-10"></div>	
	<div class="w3-row-padding">
	  <div class="w3-half"> <!-- Cleaning Log -->
       <?php $logcleaning ="Select Format(a.CCleaning, 'yyyy-MM-dd') As tglCleaning, cleaningType From containerJournal a
	         ...
	         mssql_free_result($reslog);	
       ?>	  
        <label style="font: 600 15px/25px Rajdhani, Helvetica, sans-serif;">Cleaning Log</label>	   
		<div class="height-10" style="border-top:1px solid #27ae60"></div>
		<div class="w3-row-padding">
		...
		<div class="w3-container">
          <form id="fcntCleaning" method="post">
			...
			<button type="submit" class="w3-button w3-light-grey main-button_light-blue">Cleaning Log</button>					 
		  </form>
	    </div>	   
	  </div>

	  <div class="w3-half"> <!-- Repair Log -->
	   <?php $noEstimate='';
             $tglSubmitted='';
             $DPP=0;
	         $dirNamePDF='';
             $logrepair="Select estimateID, Format(estimateDate, 'yyyy-MM-dd') As estimateDate, nilaiDPP, 
			                    Format(tanggalApprove, 'yyyy-MM-dd') As tglApprove, dirname, IsNull(totalHour,0) As totalHour
                         From   RepairHeader 
	         ...
	         $adm="";
	         if(substr($noEstimate,0,3)=="REP" || substr($noEstimate,0,3)=="DRF") { $desc="%".$kywrd."%"; }
	         else { $desc="%".$kywrd."%"; }
	         $logrepair="SELECT Top(1) userID, ISNULL(completeName,'') AS completeName 
	                     FROM   userLogAct 
				         INNER JOIN  m_Employee On empRegID=userID
	         ...
	         mssql_free_result($reslog);			   
	   ?>
	    <label style="font: 600 15px/25px Rajdhani, Helvetica, sans-serif;">Estimate/Repair Log</label>
		<div class="height-10" style="border-top:1px solid #27ae60"></div>
		<div class="w3-row-padding">
        ...
        <div class="w3-row-padding">
	     <div class="w3-half ">Finish Repair</div>
		...
		<div class="w3-row-padding">
		 <div class="w3-half">
		   <form id="fcntRepair" method="post">
		    ...
		    <input type="hidden" name="loc" value="<?php echo $loc?>" />				   
		    <button type="submit" <?php if($tglSurvey=="" && $tglSurvey!="1900-01-01") { echo "disabled"; }?> <?php if($principle=="") { echo "disabled"; }?>
			        class="w3-button w3-light-grey main-button_light-blue">Repair/Estimate Log</button>
		   </form>
	     </div>	
         <div class="w3-half">
         <?php 		
		 ...
		 ?>	  
            <form name="fuploadEOR" method="get" action="attchFile" target="popUpW" 
		      ...
		      <input type="hidden" name="noCnt" value="<?php echo $kywrd;?>" />			         
			  <input type="hidden" name="kodeBook" value="<?php echo $kodeBooking?>" />						  
			  <button type="submit" class="w3-button w3-light-grey main-button_light-blue">Upload Estimate</button>
		    </form>
		 <?php		  
		   }
		   if(trim($noEstimate)=="" && trim($principle)!="" && $workshopID!="" && $tglSurvey!="" && $tglSurvey!="1900-01-01") {
			echo '<form id="fCreateEOR" method="get">
			        ...
			        <button type="submit" class="w3-button w3-light-grey main-button_light-blue">Create Estimate</button>	
                  </form>';
           }					                    
         ...
         ?>			   
			<form id="fCreateEOR" method="post">
			        ...
			        <button type="submit" <?php if($principle=="") { echo "disabled"; }?> class="w3-button w3-light-grey main-button_light-blue">Review Estimate</button>	
            </form>					
		 <?php 		  
           }							
		 ?>  		 
