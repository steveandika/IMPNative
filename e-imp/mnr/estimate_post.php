...
*/
  ...
  if(isset($_POST['laborDepo']))   { $labour=$_POST['laborDepo']; }
  ...
  if($to_do != "" && $noCnt != "") {    
	...
	for($i=0; $i<$totalBaris; $i++) 
	{
      ...
      if($chk[$i] == "on") 
	  {
		...
		$laborDepo=$_POST['laborDepo'];
		if($tipe=='GP') { $isType='DV'; }
		   
		$isFalse=0; /* var for counter */
		
/*		if($_POST['loc'][$i] != '' && $_POST['part'][$i] != '' && $_POST['dmg'][$i] != '' && $_POST['repair'][$i] != '' &&
		   $_POST['length'][$i] != '') */
		if($_POST['loc'][$i] != '' && $_POST['repair'][$i] != '' &&  $_POST['qty'][$i] != '') 		   
		{
